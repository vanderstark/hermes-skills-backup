...
| **Git push fails: 'Repository not found' / auth error** | Two fixes: (1) Ensure branch matches remote — use `git push -u origin master` if local is `master` (GitHub's `auto_init:true` creates `master`, not `main`). (2) For headless/agent environments, embed PAT in remote URL: `git remote set-url origin https://<TOKEN>@github.com/owner/repo.git`, push, then **immediately** `rm /tmp/gh_token_file`. Never assume branch name — check `git branch` first. |\n| **Token security** | Always store PAT in `/tmp/gh_token_file` with `chmod 600`. NEVER log tokens inline in logs. Delete immediately after push. Instruct user to **revoke token** at github.com/settings/tokens. |\n---\n\n## 🎯 Multi-Lab Roadmap (LAB-001 to LAB-068)

...
Load these on demand for specific situations — don't read them all upfront:

- **`references/root-cause-tracing.md`** — Trace a bug backward through the call stack to find the original trigger, when the error surfaces far from its source.
- **`references/defense-in-depth.md`** — After finding the root cause, add validation at multiple layers so the same class of bug can't recur silently.
- **`references/condition-based-waiting.md`** (+ `condition-based-waiting-example.ts`) — Replace arbitrary `sleep`/timeout waits with condition polling, for flaky async/timing bugs.
- **`references/find-polluter.sh`** — Bisection script to find which test in a suite is polluting state for another (shared-state test pollution).

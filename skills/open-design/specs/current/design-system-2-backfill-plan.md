...
OpenDesign now has two related but different design-system states:
...
This is not a claim that OpenDesign has re-crawled every original upstream
brand repository. For bundled systems, provenance should be explicit: the
derived package is based on the curated OpenDesign design-system fixture. That

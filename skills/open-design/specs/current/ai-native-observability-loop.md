...
Define the target observability loop that lets OpenDesign improve agent
...
OpenDesign already reports completed runs to Langfuse and PostHog. The current
...
That means OpenDesign can inspect individual runs, but the system does not yet
...
The gap is therefore not "does OpenDesign know how to talk to object storage?"
...
OpenDesign should treat observability as a closed loop:
...
alerting surface. OpenDesign owns the domain model that maps a design-agent run
...
OpenDesign is a design product, so the input and output surface is heavier than
...
| --- | --- |
| OpenDesign object storage | Stores original user attachments, parsed text, thumbnails, OCR or embedding outputs, generated artifacts, preview screenshots, export bundles, and reproducibility files. |
...
- always emit an `attachment_manifest` and `artifact_manifest` with stable ids,
  ...
  redaction status, and OpenDesign storage references;
...
OpenDesign should add a registry layer above `ProjectStorage` instead of
...
apply, OpenDesign should not write a score value; it should record
...
- Define the OpenDesign object storage contract for attachments, parsed
...
- Store original heavy inputs and outputs in OpenDesign controlled storage,

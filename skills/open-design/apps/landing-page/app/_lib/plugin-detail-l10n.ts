...
> = {
  zh: {
    ...
    systemsMdBody: [
      ...
      '把 Claude Code、Cursor 或任意编程 agent 指向这个文件，它生成的每个组件、页面和素材都会继承相同的品牌身份。DESIGN.md 是一种开放的 Apache-2.0 格式；OpenDesign 则是围绕它构建的开源、本地优先的库与工具链。',
    ],
    ...
    systemsMdSpecNote:
      'DESIGN.md 是一种开放格式（Apache-2.0）。OpenDesign 的系统可在 GitHub 上自由阅读、fork 和贡献。',
    systemsFaqHead: '常见问题',
    systemsFaq: (n) => [
      ...
      { q: '如何在 Claude Code 或 Cursor 中使用 DESIGN.md？', a: '把文件保存到项目根目录，并让你的 agent 遵循它。OpenDesign 还能把项目绑定到某套系统，让每一份插件输出自动继承相同的品牌身份。' },
      { q: '这些设计系统是免费的吗？', a: '是的。这里的每套系统都是开源的，可自由阅读、下载、fork 和贡献。OpenDesign 本身采用 Apache-2.0 协议，且本地优先。' },
      { q: '一共有多少套设计系统？', a: `${n} 套且持续增长，涵盖消费科技、编辑排版和实验性风格。新系统会定期上线，你也可以在 GitHub 上贡献自己的系统。` },
      { q: '我能创建自己的 DESIGN.md 吗？', a: '可以——手写一份 DESIGN.md，或让 OpenDesign 从一个参考网站生成一份，然后在每个项目和 agent 中复用它。' },
    ],
    tokensHead: '设计 token',
    tokensLead: (n) =>
      `${n} 个 token，符合 OpenDesign 的 token 契约——你的 agent 读取的正是这套结构化的配色、字体、间距和动效值，用以为任意产物设定主题。`,
    ...
    detailSystemTitleSuffix: '为你的 agent 准备的配色、字体与 token · OpenDesign',
  ...
  'zh-tw': {
    ...
    systemsMdBody: [
      ...
      '讓 Claude Code、Cursor 或任何 coding agent 指向這份檔案，它所產生的每一個元件、頁面與素材都會繼承相同的識別。DESIGN.md 是一種開放的 Apache-2.0 格式；OpenDesign 則是圍繞它打造的開源、本地優先函式庫與工具鏈。',
    ],
    ...
    systemsMdSpecNote:
      'DESIGN.md 是一種開放格式（Apache-2.0）。OpenDesign 的各套系統都可在 GitHub 上自由閱讀、fork 與貢獻。',
    systemsFaqHead: '常見問題',
    systemsFaq: (n) => [
      ...
      { q: '如何在 Claude Code 或 Cursor 中使用 DESIGN.md？', a: '把檔案存到專案根目錄，並告訴你的 agent 去遵循它。OpenDesign 也能將專案綁定到某套系統，讓每一份 plugin 產出都自動繼承相同的識別。' },
      { q: '這些設計系統免費嗎？', a: '是的。這裡的每一套系統都是開源且可自由閱讀、下載、fork 與貢獻的。OpenDesign 本身採用 Apache-2.0，並以本地優先為原則。' },
      { q: '一共有多少套設計系統？', a: `${n} 套且持續增加，橫跨消費科技、編輯風格與實驗性風格。新系統會定期上架，你也可以在 GitHub 上貢獻自己的作品。` },
      { q: '我可以建立自己的 DESIGN.md 嗎？', a: '可以——你可以手動撰寫一份 DESIGN.md，或讓 OpenDesign 從參考網站生成一份，再於每個專案與 agent 之間重複使用。' },
    ],
    tokensHead: '設計 token',
    tokensLead: (n) =>
      `${n} 個符合 OpenDesign token 規約的 token——也就是你的 agent 用來為任何產物套用主題時所讀取的同一組結構化色盤、字型、間距與動態數值。`,
    ...
    detailSystemTitleSuffix: '為你的 agent 提供的色盤、字體排印與 token · OpenDesign',
  ...
  ja: {
    ...
    systemsAboutBody: 'デザインシステムとは、カラーパレット、タイポグラフィ、スペーシング、モーション、ボイスなど、すべての画面を一貫させるための再利用可能なブランド基盤のセットです。OpenDesign では各デザインシステムはプラグインとして提供されています。プロジェクトにスナップするだけで、コーディングエージェントがパレット、タイプ、モーション、ボイスを自動的に継承し、生成されるすべてのものがブランドに沿ったものになります。',
    ...
    systemsMdBody: [
      ...
      'Claude Code、Cursor、あるいは任意のコーディング agent をこのファイルに向けるだけで、生成されるすべてのコンポーネント、ページ、アセットが同じアイデンティティを継承します。DESIGN.md はオープンな Apache-2.0 フォーマットであり、OpenDesign はそれを中心に構築されたオープンソースかつローカルファーストのライブラリとツール群です。',
    ],
    ...
    systemsMdSpecNote:
      'DESIGN.md はオープンフォーマットです（Apache-2.0）。OpenDesign のシステムは GitHub で自由に読み、フォークし、貢献できます。',
    ...
    systemsFaq: (n) => [
      ...
      { q: 'Claude Code や Cursor で DESIGN.md をどう使いますか？', a: 'ファイルをプロジェクトのルートに保存し、agent にそれに従うよう指示します。OpenDesign ではプロジェクトをシステムにスナップすることもでき、すべてのプラグイン出力が自動的に同じアイデンティティを継承します。' },
      { q: 'これらのデザインシステムは無料ですか？', a: 'はい。ここにあるすべてのシステムはオープンソースで、自由に読み、ダウンロードし、フォークし、貢献できます。OpenDesign 自体も Apache-2.0 でローカルファーストです。' },
      { q: 'デザインシステムはいくつありますか？', a: `現在 ${n} 件あり、さらに増え続けています。コンシューマーテック、エディトリアル、実験的なスタイルにわたります。新しいシステムは定期的に追加され、GitHub であなた自身のものを貢献できます。` },
      { q: '自分の DESIGN.md を作成できますか？', a: 'はい。DESIGN.md を手で書くこともできますし、OpenDesign に参照サイトから生成させることもできます。そして、それをすべてのプロジェクトと agent で再利用できます。' },
    ],
    tokensHead: 'デザイントークン',
    tokensLead: (n) =>
      `OpenDesign のトークン契約に準拠した ${n} 個のトークン。agent があらゆるアーティファクトをテーマ化するために読む、構造化されたパレット、タイプ、スペーシング、モーションの値そのものです。`,
    ...
    detailSystemTitleSuffix: 'agent のためのパレット、タイポグラフィ、トークン · OpenDesign',
  ...
  ko: {
    ...
    systemsAboutBody: '디자인 시스템은 컬러 팔레트, 타이포그래피, 간격, 모션, 보이스 등 모든 화면을 일관되게 유지하는 재사용 가능한 브랜드 기반의 집합입니다. OpenDesign에서 각 디자인 시스템은 플러그인입니다. 프로젝트에 연결하면 코딩 에이전트가 팔레트, 타입, 모션, 보이스를 자동으로 상속하여 생성되는 모든 것이 브랜드에 맞게 유지됩니다.',
    ...
    systemsMdBody: [
      ...
      'Claude Code, Cursor 또는 어떤 코딩 agent든 이 파일을 가리키게 하면, 생성되는 모든 컴포넌트, 페이지, 에셋이 동일한 정체성을 물려받습니다. DESIGN.md는 개방형 Apache-2.0 포맷이며, OpenDesign은 그 위에 구축된 오픈소스이자 로컬 우선 라이브러리이자 도구입니다.',
    ],
    ...
    systemsMdSpecNote:
      'DESIGN.md는 개방형 포맷(Apache-2.0)입니다. OpenDesign의 시스템은 GitHub에서 자유롭게 읽고, 포크하고, 기여할 수 있습니다.',
    ...
    systemsFaq: (n) => [
      ...
      { q: 'Claude Code나 Cursor에서 DESIGN.md를 어떻게 사용하나요?', a: '파일을 프로젝트 루트에 저장하고 agent에게 이를 따르라고 지시하세요. OpenDesign은 프로젝트를 시스템에 스냅할 수도 있어, 모든 플러그인 결과물이 동일한 정체성을 자동으로 물려받습니다.' },
      { q: '이 디자인 시스템들은 무료인가요?', a: '네. 여기 있는 모든 시스템은 오픈소스이며 자유롭게 읽고, 다운로드하고, 포크하고, 기여할 수 있습니다. OpenDesign 자체도 Apache-2.0이자 로컬 우선입니다.' },
      { q: '디자인 시스템은 몇 개나 있나요?', a: `현재 ${n}개이며 계속 늘어나고 있고, 소비자 테크, 에디토리얼, 실험적 스타일을 아우릅니다. 새로운 시스템이 정기적으로 추가되며, GitHub에서 직접 기여할 수도 있습니다.` },
      { q: '나만의 DESIGN.md를 만들 수 있나요?', a: '네. DESIGN.md를 직접 작성하거나, OpenDesign이 레퍼런스 사이트를 바탕으로 생성하게 한 다음, 모든 프로젝트와 agent에 재사용하세요.' },
    ],
    tokensHead: '디자인 토큰',
    tokensLead: (n) =>
      `OpenDesign 토큰 규약을 따르는 ${n}개의 토큰 — agent가 어떤 아티팩트든 테마를 입힐 때 읽는 것과 동일한, 구조화된 팔레트, 타입, 간격, 모션 값입니다.`,
    ...
    detailSystemTitleSuffix: 'agent를 위한 팔레트, 타이포그래피 & 토큰 · OpenDesign',
  ...
  de: {
    ...
    systemsMdBody: [
      ...
      'Richte Claude Code, Cursor oder einen beliebigen Coding-Agent auf die Datei, und jede Komponente, Seite und jedes Asset, das er generiert, erbt dieselbe Identität. DESIGN.md ist ein offenes Apache-2.0-Format; OpenDesign ist die quelloffene, local-first Bibliothek und das Tooling darum herum.',
    ],
    ...
    systemsMdSpecNote:
      'DESIGN.md ist ein offenes Format (Apache-2.0). Die Systeme von OpenDesign sind frei zum Lesen, Forken und Mitwirken auf GitHub.',
    ...
    systemsFaq: (n) => [
      ...
      { q: 'Wie verwende ich eine DESIGN.md mit Claude Code oder Cursor?', a: 'Speichere die Datei im Wurzelverzeichnis deines Projekts und sag deinem Agent, dass er ihr folgen soll. OpenDesign kann ein Projekt auch an ein System koppeln, sodass jede Plugin-Ausgabe automatisch dieselbe Identität erbt.' },
      { q: 'Sind diese Designsysteme kostenlos?', a: 'Ja. Jedes System hier ist quelloffen und kostenlos zum Lesen, Herunterladen, Forken und Mitwirken. OpenDesign selbst ist Apache-2.0 und local-first.' },
      ...
      { q: 'Kann ich meine eigene DESIGN.md erstellen?', a: 'Ja – erstelle eine DESIGN.md von Hand oder lass OpenDesign eine aus einer Referenz-Website generieren und verwende sie dann projekt- und agentübergreifend wieder.' },
    ],
    tokensHead: 'Design-Tokens',
    tokensLead: (n) =>
      `${n} Tokens, die dem OpenDesign Token-Vertrag entsprechen – dieselben strukturierten Werte für Palette, Typografie, Abstände und Motion, die dein Agent liest, um jedes Artefakt zu gestalten.`,
    ...
    detailSystemTitleSuffix: 'Palette, Typografie & Tokens für deinen Agent · OpenDesign',
  ...
  fr: {
    ...
    systemsMdBody: [
      ...
      'Pointez Claude Code, Cursor ou n’importe quel agent de code vers ce fichier, et chaque composant, page et ressource qu’il génère hérite de la même identité. DESIGN.md est un format ouvert sous licence Apache-2.0 ; OpenDesign est la bibliothèque et l’outillage open source, local-first, construits autour de lui.',
    ],
    ...
    systemsMdSpecNote:
      'DESIGN.md est un format ouvert (Apache-2.0). Les systèmes d’OpenDesign sont libres à lire, à forker et à enrichir sur GitHub.',
    ...
    systemsFaq: (n) => [
      ...
      { q: 'Comment utiliser un DESIGN.md avec Claude Code ou Cursor ?', a: 'Enregistrez le fichier à la racine de votre projet et demandez à votre agent de le suivre. OpenDesign peut aussi rattacher un projet à un système, pour que chaque résultat de plugin hérite automatiquement de la même identité.' },
      { q: 'Ces systèmes de design sont-ils gratuits ?', a: 'Oui. Chaque système présenté ici est open source et libre à lire, télécharger, forker et enrichir. OpenDesign lui-même est sous licence Apache-2.0 et local-first.' },
      ...
      { q: 'Puis-je créer mon propre DESIGN.md ?', a: 'Oui — rédigez un DESIGN.md à la main, ou laissez OpenDesign en générer un à partir d’un site de référence, puis réutilisez-le sur chaque projet et chaque agent.' },
    ],
    tokensHead: 'Tokens de design',
    tokensLead: (n) =>
      `${n} tokens conformes au contrat de tokens d’OpenDesign — les mêmes valeurs structurées de palette, de typographie, d’espacement et de mouvement que votre agent lit pour thématiser n’importe quel artefact.`,
    ...
    detailSystemTitleSuffix: 'palette, typographie et tokens pour votre agent · OpenDesign',
  ...
  ru: {
    ...
    systemsMdBody: [
      ...
      'Укажите Claude Code, Cursor или любому кодинг-agent-у на этот файл — и каждый компонент, страница и ассет, который он создаёт, наследует ту же идентичность. DESIGN.md — это открытый формат под лицензией Apache-2.0; OpenDesign — это построенные вокруг него библиотека и инструменты с открытым исходным кодом и приоритетом локальной работы.',
    ],
    ...
    systemsMdSpecNote:
      'DESIGN.md — это открытый формат (Apache-2.0). Системы OpenDesign можно свободно читать, форкать и дополнять на GitHub.',
    ...
    systemsFaq: (n) => [
      ...
      { q: 'Как использовать DESIGN.md с Claude Code или Cursor?', a: 'Сохраните файл в корень проекта и скажите своему agent-у следовать ему. OpenDesign также может привязать проект к системе, чтобы каждый результат плагина автоматически наследовал ту же идентичность.' },
      { q: 'Эти дизайн-системы бесплатны?', a: 'Да. Каждая система здесь — с открытым исходным кодом, её можно свободно читать, скачивать, форкать и дополнять. Сам OpenDesign распространяется под Apache-2.0 и ориентирован на локальную работу.' },
      ...
      { q: 'Могу ли я создать собственный DESIGN.md?', a: 'Да — напишите DESIGN.md вручную или позвольте OpenDesign сгенерировать его из эталонного сайта, а затем переиспользуйте во всех проектах и agent-ах.' },
    ],
    tokensHead: 'Дизайн-токены',
    tokensLead: (n) =>
      `${n} токенов, соответствующих контракту токенов OpenDesign — та же структурированная палитра, шрифты, отступы и значения анимации, которые ваш agent читает, чтобы оформить любой артефакт.`,
    ...
    detailSystemTitleSuffix: 'палитра, типографика и токены для вашего agent-а · OpenDesign',
  ...
  es: {
    ...
    systemsMdSpecNote:
      'DESIGN.md es un formato abierto (Apache-2.0). Los sistemas de OpenDesign son libres de leer, bifurcar y contribuir en GitHub.',
    ...
    systemsFaq: (n) => [
      ...
      { q: '¿Cómo uso un DESIGN.md con Claude Code o Cursor?', a: 'Guarda el archivo en la raíz de tu proyecto y dile a tu agente que lo siga. OpenDesign también puede vincular un proyecto a un sistema para que cada resultado de los plugins herede la misma identidad automáticamente.' },
      { q: '¿Son gratuitos estos sistemas de diseño?', a: 'Sí. Cada sistema aquí es de código abierto y libre de leer, descargar, bifurcar y contribuir. OpenDesign en sí mismo es Apache-2.0 y local-first.' },
      ...
      { q: '¿Puedo crear mi propio DESIGN.md?', a: 'Sí: escribe un DESIGN.md a mano, o deja que OpenDesign genere uno a partir de un sitio de referencia, y luego reutilízalo en cada proyecto y agente.' },
    ],
    tokensHead: 'Tokens de diseño',
    tokensLead: (n) =>
      `${n} tokens que cumplen con el contrato de tokens de OpenDesign: los mismos valores estructurados de paleta, tipografía, espaciado y movimiento que tu agente lee para dar tema a cualquier artefacto.`,
    ...
    detailSystemTitleSuffix: 'paleta, tipografía y tokens para tu agente · OpenDesign',
  ...
  'pt-br': {
    ...
    systemsMdBody: [
      ...
      'Aponte o Claude Code, o Cursor ou qualquer coding agent para o arquivo e todo componente, página e asset que ele gerar herda a mesma identidade. DESIGN.md é um formato aberto Apache-2.0; o OpenDesign é a biblioteca e as ferramentas open-source, local-first, construídas em torno dele.',
    ],
    ...
    systemsMdSpecNote:
      'DESIGN.md é um formato aberto (Apache-2.0). Os sistemas do OpenDesign são livres para ler, fazer fork e contribuir no GitHub.',
    ...
    systemsFaq: (n) => [
      ...
      { q: 'Como uso um DESIGN.md com o Claude Code ou o Cursor?', a: 'Salve o arquivo na raiz do seu projeto e diga ao seu agent para segui-lo. O OpenDesign também pode vincular um projeto a um sistema para que toda saída de plugin herde a mesma identidade automaticamente.' },
      { q: 'Esses design systems são gratuitos?', a: 'Sim. Todo sistema aqui é open source e gratuito para ler, baixar, fazer fork e contribuir. O próprio OpenDesign é Apache-2.0 e local-first.' },
      ...
      { q: 'Posso criar meu próprio DESIGN.md?', a: 'Sim — escreva um DESIGN.md à mão, ou deixe o OpenDesign gerar um a partir de um site de referência e depois reutilize-o em todos os projetos e agents.' },
    ],
    tokensHead: 'Design tokens',
    tokensLead: (n) =>
      `${n} tokens em conformidade com o contrato de tokens do OpenDesign — os mesmos valores estruturados de paleta, tipografia, espaçamento e movimento que seu agent lê para tematizar qualquer artefato.`,
    ...
    detailSystemTitleSuffix: 'paleta, tipografia e tokens para o seu agent · OpenDesign',
  ...
  it: {
    ...
    systemsMdBody: [
      ...
      'Punta Claude Code, Cursor o qualsiasi coding agent al file e ogni componente, pagina e asset che genera eredita la stessa identità. DESIGN.md è un formato aperto, Apache-2.0; OpenDesign è la libreria e gli strumenti open-source e local-first costruiti attorno ad esso.',
    ],
    ...
    systemsMdSpecNote:
      'DESIGN.md è un formato aperto (Apache-2.0). I sistemi di OpenDesign sono liberi da leggere, forkare e a cui contribuire su GitHub.',
    ...
    systemsFaq: (n) => [
      ...
      { q: 'Come uso un DESIGN.md con Claude Code o Cursor?', a: 'Salva il file nella radice del tuo progetto e di’ al tuo agent di seguirlo. OpenDesign può anche agganciare un progetto a un sistema così ogni output dei plugin eredita automaticamente la stessa identità.' },
      { q: 'Questi design system sono gratuiti?', a: 'Sì. Ogni sistema qui è open source e libero da leggere, scaricare, forkare e a cui contribuire. OpenDesign stesso è Apache-2.0 e local-first.' },
      ...
      { q: 'Posso creare il mio DESIGN.md?', a: 'Sì — scrivi un DESIGN.md a mano, oppure lascia che OpenDesign ne generi uno da un sito di riferimento, poi riutilizzalo in ogni progetto e agent.' },
    ],
    tokensHead: 'Design token',
    tokensLead: (n) =>
      `${n} token conformi al token contract di OpenDesign — gli stessi valori strutturati di palette, tipografia, spaziatura e motion che il tuo agent legge per applicare il tema a qualsiasi artefatto.`,
    ...
    detailSystemTitleSuffix: 'palette, tipografia e token per il tuo agent · OpenDesign',
  ...
  vi: {
    ...
    systemsMdBody: [
      ...
      'Trỏ Claude Code, Cursor hay bất kỳ coding agent nào vào tệp này và mọi thành phần, trang cùng tài nguyên mà nó tạo ra đều kế thừa cùng một bản sắc. DESIGN.md là định dạng mở, theo giấy phép Apache-2.0; OpenDesign là thư viện và bộ công cụ mã nguồn mở, ưu tiên cục bộ được xây dựng quanh nó.',
    ],
    ...
    systemsMdSpecNote:
      'DESIGN.md là một định dạng mở (Apache-2.0). Các hệ thống của OpenDesign được tự do đọc, fork và đóng góp trên GitHub.',
    ...
    systemsFaq: (n) => [
      ...
      { q: 'Tôi dùng DESIGN.md với Claude Code hoặc Cursor như thế nào?', a: 'Lưu tệp vào thư mục gốc của dự án và bảo agent của bạn tuân theo nó. OpenDesign cũng có thể gắn một dự án với một hệ thống để mọi kết quả đầu ra của plugin tự động kế thừa cùng một bản sắc.' },
      { q: 'Các hệ thống thiết kế này có miễn phí không?', a: 'Có. Mọi hệ thống ở đây đều là mã nguồn mở và miễn phí để đọc, tải xuống, fork và đóng góp. Bản thân OpenDesign theo giấy phép Apache-2.0 và ưu tiên cục bộ.' },
      ...
      { q: 'Tôi có thể tạo DESIGN.md của riêng mình không?', a: 'Có — tự tay viết một DESIGN.md, hoặc để OpenDesign tạo một bản từ một trang tham chiếu, rồi tái sử dụng nó trên mọi dự án và agent.' },
    ],
    tokensHead: 'Token thiết kế',
    tokensLead: (n) =>
      `${n} token tuân theo bản giao kèo token của OpenDesign — cùng các giá trị bảng màu, kiểu chữ, khoảng cách và chuyển động có cấu trúc mà agent của bạn đọc để áp chủ đề cho bất kỳ tài nguyên nào.`,
    ...
    detailSystemTitleSuffix: 'bảng màu, kiểu chữ & token cho agent của bạn · OpenDesign',
  ...
  pl: {
    ...
    systemsAboutBody: 'System projektowy to wielokrotnego użytku zestaw fundamentów marki — paleta kolorów, typografia, odstępy, motion i głos — który zapewnia spójność na każdym ekranie. W OpenDesign każdy system projektowy jest pluginem: podepnij do niego projekt, a Twój coding agent automatycznie odziedziczy paletę, typografię, motion i głos, dzięki czemu wszystko, co wygeneruje, będzie zgodne z marką.',
    ...
    systemsMdBody: [
      ...
      'Skieruj Claude Code, Cursor lub dowolnego agenta kodującego na ten plik, a każdy komponent, strona i zasób, który wygeneruje, odziedziczy tę samą tożsamość. DESIGN.md to otwarty format na licencji Apache-2.0; OpenDesign to oparta na otwartym kodzie, działająca lokalnie biblioteka i narzędzia zbudowane wokół niego.',
    ],
    ...
    systemsMdSpecNote:
      'DESIGN.md to otwarty format (Apache-2.0). Systemy OpenDesign można swobodnie czytać, forkować i współtworzyć na GitHubie.',
    ...
    systemsFaq: (n) => [
      ...
      { q: 'Jak używać DESIGN.md z Claude Code lub Cursor?', a: 'Zapisz plik w katalogu głównym projektu i powiedz swojemu agentowi, aby się go trzymał. OpenDesign może też przypiąć projekt do systemu, tak aby każdy wynik wtyczki automatycznie dziedziczył tę samą tożsamość.' },
      { q: 'Czy te systemy projektowe są darmowe?', a: 'Tak. Każdy system tutaj jest oparty na otwartym kodzie i można go bezpłatnie czytać, pobierać, forkować i współtworzyć. Samo OpenDesign jest na licencji Apache-2.0 i działa lokalnie.' },
      ...
      { q: 'Czy mogę stworzyć własny DESIGN.md?', a: 'Tak — napisz DESIGN.md ręcznie albo pozwól, by OpenDesign wygenerowało go na podstawie strony referencyjnej, a następnie używaj go ponownie w każdym projekcie i u każdego agenta.' },
    ],
    tokensHead: 'Tokeny projektowe',
    tokensLead: (n) =>
      `${n} tokenów zgodnych z kontraktem tokenów OpenDesign — te same uporządkowane wartości palety, typografii, odstępów i ruchu, które Twój agent odczytuje, aby nadać motyw dowolnemu artefaktowi.`,
    ...
    detailSystemTitleSuffix: 'paleta, typografia i tokeny dla Twojego agenta · OpenDesign',
  ...
  id: {
    ...
    systemsAboutBody: 'Design system adalah sekumpulan fondasi brand yang dapat digunakan kembali — palet warna, tipografi, spasi, motion, dan voice — yang menjaga konsistensi setiap layar. Di OpenDesign, setiap design system adalah sebuah plugin: hubungkan proyek ke salah satunya dan coding agent Anda akan otomatis mewarisi palet, tipografi, motion, dan voice, sehingga semua yang dihasilkannya tetap sesuai brand.',
    detailSystemLabel: 'design system',
    ...
    systemsMdSpecNote:
      'DESIGN.md adalah format terbuka (Apache-2.0). Sistem-sistem OpenDesign bebas dibaca, di-fork, dan dikontribusikan di GitHub.',
    ...
    systemsFaq: (n) => [
      ...
      { q: 'Bagaimana cara menggunakan DESIGN.md dengan Claude Code atau Cursor?', a: 'Simpan file itu ke root proyek Anda dan minta agent Anda untuk mengikutinya. OpenDesign juga bisa menautkan sebuah proyek ke suatu sistem agar setiap keluaran plugin mewarisi identitas yang sama secara otomatis.' },
      { q: 'Apakah design system ini gratis?', a: 'Ya. Setiap sistem di sini open source dan gratis untuk dibaca, diunduh, di-fork, dan dikontribusikan. OpenDesign sendiri berlisensi Apache-2.0 dan mengutamakan local-first.' },
      ...
      { q: 'Bisakah saya membuat DESIGN.md sendiri?', a: 'Bisa — tulis DESIGN.md secara manual, atau biarkan OpenDesign menghasilkannya dari sebuah situs referensi, lalu gunakan kembali di setiap proyek dan agent.' },
    ],
    tokensHead: 'Design token',
    tokensLead: (n) =>
      `${n} token yang mengikuti kontrak token OpenDesign — palet, tipe, spasi, dan nilai gerak terstruktur yang sama yang dibaca agent Anda untuk menata tema artefak apa pun.`,
    ...
    detailSystemTitleSuffix: 'palet, tipografi & token untuk agent Anda · OpenDesign',
  ...
  nl: {
    ...
    systemsAboutBody: 'Een design system is een herbruikbare set merkfundamenten — kleurpalet, typografie, witruimte, motion en voice — die elk scherm consistent houdt. In OpenDesign is elk design system een plugin: koppel een project eraan en je coding agent erft automatisch het palet, de typografie, motion en voice, zodat alles wat het genereert on-brand blijft.',
    ...
    systemsMdBody: [
      ...
      'Wijs Claude Code, Cursor of een willekeurige coding-agent naar het bestand en elk component, elke pagina en elke asset die wordt gegenereerd, erft dezelfde identiteit. DESIGN.md is een open, Apache-2.0-formaat; OpenDesign is de open-source, local-first bibliotheek en tooling die eromheen is gebouwd.',
    ],
    ...
    systemsMdSpecNote:
      'DESIGN.md is een open formaat (Apache-2.0). De systemen van OpenDesign zijn vrij te lezen, te forken en aan bij te dragen op GitHub.',
    ...
    systemsFaq: (n) => [
      ...
      { q: 'Hoe gebruik ik een DESIGN.md met Claude Code of Cursor?', a: 'Sla het bestand op in de root van je project en zeg je agent het te volgen. OpenDesign kan een project ook koppelen aan een systeem, zodat elke plugin-output automatisch dezelfde identiteit erft.' },
      { q: 'Zijn deze designsystemen gratis?', a: 'Ja. Elk systeem hier is open source en gratis te lezen, te downloaden, te forken en aan bij te dragen. OpenDesign zelf is Apache-2.0 en local-first.' },
      ...
      { q: 'Kan ik mijn eigen DESIGN.md maken?', a: 'Ja — schrijf een DESIGN.md met de hand, of laat OpenDesign er een genereren op basis van een referentiesite, en hergebruik die vervolgens in elk project en met elke agent.' },
    ],
    tokensHead: 'Design-tokens',
    tokensLead: (n) =>
      `${n} tokens die voldoen aan het OpenDesign token-contract — dezelfde gestructureerde palet-, type-, witruimte- en bewegingswaarden die je agent leest om elk artefact te thematiseren.`,
    ...
    detailSystemTitleSuffix: 'palet, typografie & tokens voor je agent · OpenDesign',
  ...
  ar: {
    ...
    systemsAboutBody: 'نظام التصميم هو مجموعة قابلة لإعادة الاستخدام من أسس العلامة التجارية — لوحة الألوان والطباعة والتباعد والحركة والأسلوب — تحافظ على اتساق كل شاشة. في OpenDesign، كل نظام تصميم هو إضافة: اربط مشروعًا به وسيرث وكيل الترميز لديك تلقائيًا لوحة الألوان والطباعة والحركة والأسلوب، ليبقى كل ما يُنشئه متوافقًا مع هوية العلامة التجارية.',
    ...
    systemsMdBody: [
      ...
      'وجّه Claude Code أو Cursor أو أي coding agent إلى الملف، فترث كل مكوّن وصفحة وأصل يُولّد الهوية ذاتها. صيغة DESIGN.md مفتوحة بترخيص Apache-2.0؛ وOpenDesign هي المكتبة والأدوات مفتوحة المصدر والمحلية أولاً المبنية حولها.',
    ],
    ...
    systemsMdSpecNote:
      'DESIGN.md صيغة مفتوحة (Apache-2.0). أنظمة OpenDesign متاحة للقراءة والاشتقاق والمساهمة فيها مجانًا على GitHub.',
    ...
    systemsFaq: (n) => [
      ...
      { q: 'كيف أستخدم ملف DESIGN.md مع Claude Code أو Cursor؟', a: 'احفظ الملف في جذر مشروعك واطلب من الـ agent اتّباعه. يمكن لـ OpenDesign أيضًا ربط مشروع بنظام بحيث يرث كل ناتج plugin الهوية ذاتها تلقائيًا.' },
      { q: 'هل أنظمة التصميم هذه مجانية؟', a: 'نعم. كل نظام هنا مفتوح المصدر ومتاح للقراءة والتنزيل والاشتقاق والمساهمة فيه مجانًا. وOpenDesign نفسها بترخيص Apache-2.0 ومحلية أولاً.' },
      ...
      { q: 'هل يمكنني إنشاء ملف DESIGN.md خاص بي؟', a: 'نعم — اكتب ملف DESIGN.md يدويًا، أو دع OpenDesign يولّد واحدًا من موقع مرجعي، ثم أعد استخدامه عبر كل مشروع وكل agent.' },
    ],
    tokensHead: 'رموز التصميم',
    tokensLead: (n) =>
      `${n} من الرموز المتوافقة مع عقد الرموز في OpenDesign — نفس قيم لوحة الألوان والخطوط والمسافات والحركة المنظّمة التي يقرؤها الـ agent لتنسيق أي مُخرَج.`,
    ...
    detailSystemTitleSuffix: 'لوحة ألوان وخطوط ورموز للـ agent الخاص بك · OpenDesign',
  ...
  tr: {
    ...
    systemsAboutBody: 'Tasarım sistemi; her ekranı tutarlı tutan, renk paleti, tipografi, boşluk, hareket ve ses gibi yeniden kullanılabilir marka temellerinden oluşan bir kümedir. OpenDesign\'da her tasarım sistemi bir plugin\'dir: bir projeyi buna bağlayın ve kodlama ajanınız paleti, tipografiyi, hareketi ve sesi otomatik olarak devralır; böylece oluşturduğu her şey marka uyumlu kalır.',
    ...
    systemsMdBody: [
      ...
      'Claude Code’u, Cursor’ı ya da herhangi bir kodlama agent’ını bu dosyaya yönlendirin; ürettiği her bileşen, sayfa ve varlık aynı kimliği devralır. DESIGN.md açık, Apache-2.0 bir formattır; OpenDesign ise onun etrafında inşa edilmiş, açık kaynaklı ve önce-yerel çalışan kütüphane ve araç setidir.',
    ],
    ...
    systemsMdSpecNote:
      'DESIGN.md açık bir formattır (Apache-2.0). OpenDesign’ın sistemleri GitHub’da ücretsiz okunabilir, çatallanabilir ve katkıya açıktır.',
    ...
    systemsFaq: (n) => [
      ...
      { q: 'Bir DESIGN.md dosyasını Claude Code veya Cursor ile nasıl kullanırım?', a: 'Dosyayı projenizin kök dizinine kaydedin ve agent’ınıza onu izlemesini söyleyin. OpenDesign ayrıca bir projeyi bir sisteme bağlayabilir; böylece her plugin çıktısı aynı kimliği otomatik olarak devralır.' },
      { q: 'Bu tasarım sistemleri ücretsiz mi?', a: 'Evet. Buradaki her sistem açık kaynaklıdır ve okumak, indirmek, çatallamak ve katkıda bulunmak ücretsizdir. OpenDesign’ın kendisi Apache-2.0 ve önce-yerel çalışır.' },
      ...
      { q: 'Kendi DESIGN.md dosyamı oluşturabilir miyim?', a: 'Evet — bir DESIGN.md’yi elle yazabilir ya da OpenDesign’ın bir referans siteden bir tane üretmesini sağlayabilir, ardından bunu her projede ve agent’ta yeniden kullanabilirsiniz.' },
    ],
    tokensHead: 'Tasarım token’ları',
    tokensLead: (n) =>
      `OpenDesign token sözleşmesine uyan ${n} token — agent’ınızın herhangi bir çıktıyı temalandırmak için okuduğu aynı yapılandırılmış palet, tipografi, boşluk ve hareket değerleri.`,
    ...
    detailSystemTitleSuffix: 'agent’ınız için palet, tipografi ve token’lar · OpenDesign',
  ...
  uk: {
    ...
    systemsAboutBody: 'Система дизайну — це повторно використовуваний набір основ бренду: кольорова палітра, типографіка, відступи, анімація та голос, — що забезпечує узгодженість кожного екрана. В OpenDesign кожна система дизайну є плагіном: підключіть до неї проект — і ваш coding-агент автоматично успадкує палітру, типографіку, анімацію та голос, тож усе, що він генерує, залишатиметься відповідним до бренду.',
    detailSystemLabel: 'дизайн-система',
    ...
    systemsMdBody: [
      ...
      'Спрямуйте Claude Code, Cursor чи будь-якого кодувального агента на цей файл — і кожен компонент, сторінка та ресурс, який він генерує, успадковує ту саму ідентичність. DESIGN.md — це відкритий формат на ліцензії Apache-2.0; OpenDesign — це бібліотека з відкритим кодом і local-first інструментарій, побудовані навколо нього.',
    ],
    systemsMdSnippetCaption: 'Фрагмент DESIGN.md',
    systemsMdSteps: [
    ...
    systemsMdSpecNote:
      'DESIGN.md — це відкритий формат (Apache-2.0). Системи OpenDesign можна вільно читати, форкати та робити внески до них на GitHub.',
    ...
    systemsFaq: (n) => [
      ...
      { q: 'Як використати DESIGN.md з Claude Code чи Cursor?', a: 'Збережіть файл у корені проєкту й скажіть своєму агенту дотримуватися його. OpenDesign також може прив’язати проєкт до системи, щоб кожен результат плагіна автоматично успадковував ту саму ідентичність.' },
      { q: 'Чи безкоштовні ці дизайн-системи?', a: 'Так. Кожна система тут має відкритий код, і її можна вільно читати, завантажувати, форкати та робити внески. Сам OpenDesign — це Apache-2.0 і local-first.' },
      ...
      { q: 'Чи можу я створити власний DESIGN.md?', a: 'Так — напишіть DESIGN.md вручну або дозвольте OpenDesign згенерувати його з еталонного сайту, а потім повторно використовуйте його в усіх проєктах і агентах.' },
    ],
    tokensHead: 'Дизайн-токени',
    tokensLead: (n) =>
      `${n} токенів відповідно до контракту токенів OpenDesign — та сама структурована палітра, типографіка, відступи й значення руху, які ваш агент зчитує, щоб оформити будь-який артефакт.`,
    ...
    detailSystemTitleSuffix: 'палітра, типографіка й токени для вашого агента · OpenDesign',

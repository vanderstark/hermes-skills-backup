...
When writing or changing any test, adding mocks, or adding test helpers/cleanup,
load `references/writing-good-tests.md`. It covers two governing principles —
every test names the specific break it catches, and every test exercises the
real thing — plus rules for when mocking is actually justified, keeping
test-only code out of production classes, and understanding a dependency's
side effects before mocking it.
